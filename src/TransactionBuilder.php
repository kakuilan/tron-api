...
class TransactionBuilder {
    ...
    public function contractbalance($adres) {
        ...
        $feeLimit = 1000000;
        $func     = "balanceOf";
        $str      = '{"trc20_tokens":[{"name":"Tether USD","symbol":"USDT","decimals":6,"contract_address":"TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t"},{"name":"USD Coin","symbol":"USDC","decimals":6,"contract_address":"TEkxiTehnzSmSe2XqrBj4w32RUN966rdz8"},{"name":"Bitcoin","symbol":"BTC","decimals":8,"contract_address":"TN3W4H6rK2ce4vX9YnFQHwKENnHjoxb3m9"},{"name":"Ethereum","symbol":"ETH","decimals":18,"contract_address":"TRFe3hT5oYhjSZ6f3ji5FJ7YCfrkWnHRvh"}]}';
        ...
        foreach ($jsonData["trc20_tokens"] as $key => $item) {
            ...
            $balanceVal  = $balance_hex->value;
            $res         = gmp_div_qr($balanceVal, pow(10, $item["decimals"]));
            $amount      = floatval("{$res[0]}.{$res[1]}");
            $balance     = 0 + (float)number_format($amount, $item["decimals"], ".", "");
            if ($balance >= 0) {
                $trc20[] = [
                    "name"     => $item["name"],
                    "symbol"   => $item["symbol"],
